 ...
 * by me as a spare time project in winter 2013/2014.
 *
...
inline static float cose(float x){
...
void yaw_pitch_roll_to_ref(float yaw, float Pitch, float roll, Matrix* m){
  ...
  const float sinRoll = sine(halfRoll);
  const float cosRoll = cose(halfRoll);
  const float sinPitch = sine(halfPitch);
  const float cosPitch = cose(halfPitch);
  const float sinYaw = sine(halfYaw);
  const float cosYaw = cose(halfYaw);
